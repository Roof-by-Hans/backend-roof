...
CREATE TABLE IF NOT EXISTS `hansdev`.`MovimientoCuenta` (
  ...
  `fecha` DATETIME NOT NULL,
  `monto` DECIMAL(12,2) NOT NULL,
  `tipo_movimiento` ENUM('CONSUMO', 'RECARGA', 'PAGO') NOT NULL,
  `id_tipo_mov` INT(11) NULL DEFAULT NULL,
  `id_factura` INT(11) NULL DEFAULT NULL,
  `id_mov_caja` INT(11) NULL DEFAULT NULL,
  `id_usuario` INT(11) NULL DEFAULT NULL,
  `observaciones` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id_movimiento`),
  INDEX `id_factura` (`id_factura` ASC) VISIBLE,
  INDEX `fk_movcue_movcaja` (`id_mov_caja` ASC) VISIBLE,
  ...
  CONSTRAINT `MovimientoCuenta_ibfk_2`
    FOREIGN KEY (`id_factura`)
    REFERENCES `hansdev`.`Factura` (`id_factura`),
  CONSTRAINT `fk_movcue_movcaja`
    FOREIGN KEY (`id_mov_caja`)
    REFERENCES `hansdev`.`MovimientoCaja` (`id_mov_caja`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `hansdev`.`vw_mesas_con_grupo` (`id_mesa` INT, `nombre_mesa` INT, `id_grupo` INT, `nombre_grupo` INT);
