...
const registrarConsumo = async (req, res) => {
  ...
  try {
    ...
    // 8. Crear el movimiento de cuenta tipo CONSUMO
    ...
    await connection.execute(
      `INSERT INTO MovimientoCuenta (id_cliente, fecha, monto, tipo_movimiento, id_factura, observaciones)
       VALUES (?, NOW(), ?, 'CONSUMO', ?, ?)`,
      [idCliente, total, idFactura, observacionesCompletas]
    );
...
const registrarRecarga = async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      `INSERT INTO MovimientoCuenta (id_cliente, fecha, monto, tipo_movimiento, observaciones)
       VALUES (?, NOW(), ?, 'RECARGA', ?)`,
      [idCliente, montoRecarga, observacionesCompletas]
    );
...
const registrarPago = async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      `INSERT INTO MovimientoCuenta (id_cliente, fecha, monto, tipo_movimiento, id_factura, observaciones)
       VALUES (?, NOW(), ?, 'PAGO', ?, ?)`,
      [idCliente, montoPago, idFactura || null, observacionesCompletas]
    );
