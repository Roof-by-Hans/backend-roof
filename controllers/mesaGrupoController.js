 ...
 */
const getFacturaActivaGrupo = async (req, res) => {
  try {
    const idGrupo = sanitizeId(req.params.id);
    ...
    // Verificar que el grupo existe
    const grupo = await cargarGrupoDetalle(idGrupo);

    if (!grupo) {
      return respondError(res, 404, "Grupo no encontrado");
    }

    // Buscar factura pendiente del grupo
    const [facturaRows] = await promisePool.execute(
      `SELECT f.id_factura,
       ...
       WHERE f.id_grupo = ? AND f.estado = 'PENDIENTE'
    ...
    if (facturaRows.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No hay factura activa para este grupo",
      });
    }
    ...
    res.json({
      success: true,
      data: factura,
      message: "Factura activa del grupo obtenida correctamente",
    });
  } catch (error) {
    console.error("Error al obtener factura activa del grupo:", error);
    return respondError(res, 500, "Error interno del servidor", {
      error: error.message,
    });
  }
